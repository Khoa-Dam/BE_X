...
export async function refresh(req: Request, res: Response, next: NextFunction) {
    try { const { refreshToken } = RefreshDto.parse(req.body); res.json(success(await svc.refresh(refreshToken))); }
    catch (e) { next(e); }
}
export async function logout(req: Request, res: Response, next: NextFunction) {
    try { const rt = (req.body?.refreshToken as string | undefined); await svc.logout(req.user!.id, rt); res.json(success(true)); }
    catch (e) { next(e); }
}